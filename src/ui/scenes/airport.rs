...
impl AirportScene {
    pub fn render(
        ...
        eframe::egui::CentralPanel::default().show(ctx, |ui| {
            ...
            match scene_state.current_location {
                ...
                Location::FlightPlanning => Self::render_flight_planning(game_state, scene_state, ui),
        ...
        eframe::egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.separator();
                    ui.label(format!("⛽ {}/{}", game_state.player.fuel, game_state.player.max_fuel));
                    ui.separator();
                    
                    let current_weight = game_state.player.current_cargo_weight(&game_state.cargo_types);
                    ui.label(format!("📦 {}kg/{}kg", current_weight, game_state.player.max_cargo_weight));
                    ui.separator();
                    
                    ui.label(format!("📍 {}", 
                        game_state.airports.get(&game_state.player.current_airport)
                            .map(|a| &a.name)
    ...
    fn render_main_desk(game_state: &GameState, _scene_state: &SceneState, ui: &mut eframe::egui::Ui) {
        ...
        eframe::egui::Frame::none()
            ...
            .stroke(eframe::egui::Stroke::new(1.0, eframe::egui::Color32::from_gray(200)))
                ...
                eframe::egui::Grid::new("status_overview")
                    ...
                    .show(ui, |ui| {
                        ...
                        let (status_text, status_color) = if game_state.is_game_won() { 
                            ("🏆 WINNER! You've made $100,000!", eframe::egui::Color32::from_rgb(255, 215, 0))
                        } else if game_state.can_player_continue() {
                            ("✅ Active pilot - ready for business", eframe::egui::Color32::from_rgb(50, 150, 50))
                        } else {
                            ("⚠️ Low on fuel and funds - need assistance", eframe::egui::Color32::from_rgb(220, 50, 50))
                        };
                        ...
                        let current_weight = game_state.player.current_cargo_weight(&game_state.cargo_types);
                        ui.label(format!("{} types | {}kg/{} max", cargo_count, current_weight, game_state.player.max_cargo_weight));
                        ui.end_row();
                        
                        ui.label("Fuel Status:");
                        let fuel_percent = (game_state.player.fuel as f32 / game_state.player.max_fuel as f32) * 100.0;
                        ...
                        ui.colored_label(fuel_color, format!("{:.0}% capacity ({}/{})", fuel_percent, game_state.player.fuel, game_state.player.max_fuel));
                        ui.end_row();
        ...
        ui.collapsing("💡 Pilot's Handbook", |ui| {
            ...
            if game_state.cheat_mode {
                ui.separator();
                ui.colored_label(eframe::egui::Color32::from_rgb(255, 140, 0), "⚡ Cheat mode is active - unlimited fuel available!");
            }
        });
    }
                ...
                .show(ui, |ui| {
                    ...
                    for (cargo_id, price) in &market.cargo_prices {
                        if let Some(cargo_type) = game_state.cargo_types.get(cargo_id) {
                            ...
                            let price_color = if *price > cargo_type.base_price + (cargo_type.base_price / 4) {
                            ...
                            let change_percent = (((*price as f32) - (cargo_type.base_price as f32)) / (cargo_type.base_price as f32)) * 100.0;
                            ...
                            let (trend_text, trend_color) = if cargo_type.volatility > 0.4 {
                                ("📈 Very Volatile", eframe::egui::Color32::from_rgb(255, 140, 0))
                            } else if cargo_type.volatility > 0.3 {
                                ("📊 Volatile", eframe::egui::Color32::from_rgb(255, 165, 0))
                            } else if *price > cargo_type.base_price {
                                ("📈 Above Average", eframe::egui::Color32::from_rgb(220, 50, 50))
                            } else if *price < cargo_type.base_price {
                                ("📉 Below Average", eframe::egui::Color32::from_rgb(50, 150, 50))
                            } else {
    ...
    fn render_trading_desk(game_state: &mut GameState, scene_state: &mut SceneState, ui: &mut eframe::egui::Ui) {
                        ...
                        for cargo_id in market.cargo_prices.keys() {
                            if let Some(cargo_type) = game_state.cargo_types.get(cargo_id) {
                                ...
                                ui.selectable_value(
                                    ...
                                    Some(cargo_id.clone()),
                                    format!("{} {} ({}kg/unit)", icon, cargo_type.name, cargo_type.weight_per_unit)
                                );
                            }
                        }
                    }
        ...
        ui.horizontal(|ui| {
            ...
            ui.add(eframe::egui::Slider::new(&mut scene_state.trade_quantity, 1..=max_quantity.max(current_quantity)).text("units"));
            
            // Quick quantity buttons
            ui.separator();
            if ui.small_button("1").clicked() { scene_state.trade_quantity = 1; }
            if ui.small_button("5").clicked() { scene_state.trade_quantity = 5.min(max_quantity); }
            if ui.small_button("Max").clicked() { scene_state.trade_quantity = max_quantity; }
        });
        ...
        if let Some(selected_cargo_id) = &scene_state.selected_cargo {
            ...
            if let (Some(cargo_type), Some(market)) = (cargo_type, market) {
                if let Some(current_price) = market.cargo_prices.get(selected_cargo_id) {
                        ...
                        .show(ui, |ui| {
                            ...
                            eframe::egui::Grid::new("transaction_details")
                                ...
                                .show(ui, |ui| {
                                    ...
                                    let owned = game_state.player.cargo_inventory.get_quantity(selected_cargo_id);
                    ...
                    eframe::egui::Frame::none()
                        ...
                        .stroke(eframe::egui::Stroke::new(1.0, eframe::egui::Color32::from_rgb(100, 200, 100)))
                        .inner_margin(eframe::egui::Margin::same(8.0))
                        .show(ui, |ui| {
                            ui.horizontal(|ui| {
                                let can_buy = game_state.player.can_afford(total_cost) 
                                    && game_state.player.can_carry_more_weight(total_weight, &game_state.cargo_types);
                                
                                ui.add_enabled_ui(can_buy, |ui| {
                                    if ui.button(format!("💰 BUY {} units", scene_state.trade_quantity)).clicked() {
                                ...
                                if can_buy {
                                    ui.label(format!("Cost: ${} | Weight: {}kg", total_cost, total_weight));
                                    ui.separator();
                                    ui.label(format!("After: ${} remaining", game_state.player.money.saturating_sub(total_cost)));
                                } else {
                                    if !game_state.player.can_afford(total_cost) {
                                        ui.colored_label(eframe::egui::Color32::from_rgb(200, 50, 50), "💸 Not enough money");
                                    } else {
                                        ui.colored_label(eframe::egui::Color32::from_rgb(200, 50, 50), "📦 Not enough cargo space");
                                    }
                                }
                            });
                        });
                    ...
                    eframe::egui::Frame::none()
                        ...
                        .stroke(eframe::egui::Stroke::new(1.0, eframe::egui::Color32::from_rgb(200, 150, 100)))
                        .inner_margin(eframe::egui::Margin::same(8.0))
                        .show(ui, |ui| {
                            ui.horizontal(|ui| {
                                let owned_quantity = game_state.player.cargo_inventory.get_quantity(selected_cargo_id);
                                ...
                                ui.add_enabled_ui(can_sell, |ui| {
                                    if ui.button(format!("💵 SELL {} units", sell_quantity)).clicked() {
                                ...
                                if can_sell {
                                    ...
                                    ui.label(format!("Revenue: ${} | Units: {}", sell_value, sell_quantity));
                                    ui.separator();
                                    ui.label(format!("After: ${} total", game_state.player.money + sell_value));
                                } else {
                                    ui.colored_label(eframe::egui::Color32::from_rgb(200, 50, 50), "❌ No cargo to sell");
                                }
                            });
                        });
                }
    ...
    fn render_flight_planning(game_state: &mut GameState, scene_state: &mut SceneState, ui: &mut eframe::egui::Ui) {
        ...
        eframe::egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.separator();
                    ui.label(format!("Fuel: {}/{} units", game_state.player.fuel, game_state.player.max_fuel));
                    ui.separator();
                    ui.label(format!("Range: ~{:.0}km", (game_state.player.fuel as f32) * game_state.player.fuel_efficiency));
                    ui.separator();
                    let fuel_percent = (game_state.player.fuel as f32 / game_state.player.max_fuel as f32) * 100.0;
                    let fuel_color = if fuel_percent > 75.0 {
                        eframe::egui::Color32::from_rgb(50, 150, 50)
        ...
        let destinations: Vec<_> = game_state.get_available_destinations().into_iter().cloned().collect();
        ...
        eframe::egui::Grid::new("destinations_grid")
            ...
            .show(ui, |ui| {
                ...
                for airport in destinations {
                    if let Some(distance) = game_state.get_distance(&game_state.player.current_airport, &airport.id) {
                        let fuel_needed = game_state.player.fuel_needed_for_distance(distance);
                        let can_travel = game_state.player.can_travel_distance(distance) || game_state.cheat_mode;
                        ...
                        if can_travel {
                            if game_state.cheat_mode {
                                ui.colored_label(eframe::egui::Color32::from_rgb(255, 140, 0), "⚡ Cheat");
                            } else {
                                ui.colored_label(eframe::egui::Color32::from_rgb(50, 150, 50), "✅ Yes");
                            }
                        } else {
        ...
        ui.collapsing("💡 Flight Planning Tips", |ui| {
            ...
            if game_state.cheat_mode {
                ui.colored_label(eframe::egui::Color32::from_rgb(255, 140, 0), "• Cheat mode active: Unlimited fuel available");
            }
        });
    }
    
    fn render_fuel_pump(game_state: &mut GameState, scene_state: &mut SceneState, ui: &mut eframe::egui::Ui) {
        ...
        if let Some(market) = game_state.get_current_market() {
                ...
                .show(ui, |ui| {
                    ...
                    eframe::egui::Grid::new("fuel_status")
                        ...
                        .show(ui, |ui| {
                            ...
                            let fuel_percent = (game_state.player.fuel as f32 / game_state.player.max_fuel as f32) * 100.0;
                            ui.horizontal(|ui| {
                                ui.label(format!("{}/{} units", game_state.player.fuel, game_state.player.max_fuel));
                                let fuel_color = if fuel_percent > 75.0 {
                                    eframe::egui::Color32::from_rgb(50, 150, 50)
                                } else if fuel_percent > 25.0 {
                                    eframe::egui::Color32::from_rgb(255, 140, 0)
                            ...
                            let range = (game_state.player.fuel as f32) * game_state.player.fuel_efficiency;
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(eframe::egui::Slider::new(&mut scene_state.fuel_quantity, 1..=max_slider).text("units"));
                
                // Quick fuel buttons
                ui.separator();
                if ui.small_button("10").clicked() { scene_state.fuel_quantity = 10.min(max_fuel_can_add); }
                if ui.small_button("25").clicked() { scene_state.fuel_quantity = 25.min(max_fuel_can_add); }
                if ui.small_button("50").clicked() { scene_state.fuel_quantity = 50.min(max_fuel_can_add); }
                if ui.small_button("Fill").clicked() { scene_state.fuel_quantity = max_fuel_can_add; }
            });
            ...
            if actual_fuel_to_add > 0 {
                    ...
                    .show(ui, |ui| {
                        ...
                        eframe::egui::Grid::new("fuel_preview")
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.label(format!("{}/{} units ({:.0}%)", 
                                    game_state.player.fuel + actual_fuel_to_add,
                                    game_state.player.max_fuel,
                                    ((game_state.player.fuel + actual_fuel_to_add) as f32 / game_state.player.max_fuel as f32) * 100.0
                                ));
                                ui.end_row();
                                
                                ui.label("New Range:");
                                let new_range = (game_state.player.fuel + actual_fuel_to_add) as f32 * game_state.player.fuel_efficiency;
                                ui.label(format!("~{:.0} km", new_range));
                                ui.end_row();
                                
                                ui.label("Money After:");
                                ui.label(format!("${}", game_state.player.money.saturating_sub(total_cost)));
                                ui.end_row();
                            });
                    });
                ...
                ui.horizontal(|ui| {
                    ui.add_enabled_ui(can_buy, |ui| {
                        if ui.button(format!("⛽ PURCHASE {} units for ${}", actual_fuel_to_add, total_cost)).clicked() {
                            if game_state.player.spend_money(total_cost) {
                    ...
                    if !can_buy {
                        if total_cost > game_state.player.money {
                            ui.colored_label(eframe::egui::Color32::from_rgb(220, 50, 50), "💸 Not enough money");
                        }
                    } else {
