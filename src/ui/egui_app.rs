use crate::{
    ...
    ui::scenes::{SceneState, Scene, airport::AirportScene},
};
...
impl eframe::App for KzrkEguiApp {
    fn update(&mut self, ctx: &eframe::egui::Context, _frame: &mut eframe::Frame) {
        ...
        if let Scene::Airport(ref airport_id) = self.scene_state.current_scene {
            if airport_id != &self.game_state.player.current_airport {
                self.scene_state.travel_to_airport(self.game_state.player.current_airport.clone());
            }
        }
