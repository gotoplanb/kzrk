...
impl TradingSystem {
    pub fn buy_cargo(
        ...
        let cargo_type = cargo_types.get(cargo_id).ok_or(TradingError::CargoNotAvailable)?;
        let unit_price = market.get_cargo_price(cargo_id).ok_or(TradingError::CargoNotAvailable)?;
