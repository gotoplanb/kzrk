...
impl EventSystem {
    pub fn generate_random_event(
        ...
        let (multiplier, duration, description) = match event_type {
            MarketEventType::PriceSpike => {
                ...
                let desc = Self::generate_spike_description(
                    &affected_cargo,
                    &affected_airport,
                    airports,
                    cargo_types,
            ...
            MarketEventType::PriceCrash => {
                ...
                let desc = Self::generate_crash_description(
                    &affected_cargo,
                    &affected_airport,
                    airports,
                    cargo_types,
            ...
            MarketEventType::Shortage => {
                ...
                let desc = Self::generate_shortage_description(
                    &affected_cargo,
                    &affected_airport,
                    airports,
                    cargo_types,
    ...
    fn generate_spike_description(
        cargo: &str,
        airport: &str,
        airports: &HashMap<String, Airport>,
        cargo_types: &HashMap<String, CargoType>,
    ) -> String {
        let airport_name = airports
            .get(airport)
            .map(|a| a.name.as_str())
            .unwrap_or(airport);
        let cargo_name = cargo_types
            .get(cargo)
            .map(|c| c.name.as_str())
            .unwrap_or(cargo);
        ...
        let scenarios = [
            format!(
                "🔥 BREAKING: Factory fire at {} creates urgent demand for {}!",
                airport_name, cargo_name
            ),
            format!(
                "📈 MARKET ALERT: Supply chain disruption drives up {} prices at {}",
                cargo_name, airport_name
            ),
            format!(
                "⚡ URGENT: Emergency stockpiling of {} needed at {} due to regulations",
                cargo_name, airport_name
            ),
            format!(
                "🎯 HOT COMMODITY: Unexpected surge in {} demand at {} - prices soaring!",
                cargo_name, airport_name
            ),
        ];
    ...
    fn generate_crash_description(
        ...
        let scenarios = [
            format!(
                "📉 MARKET CRASH: Oversupply of {} floods {} market - prices plummet!",
                cargo_name, airport_name
            ),
            format!(
                "⬇️ PRICE ALERT: Major competitor dumps {} inventory at {} - prices collapse",
                cargo_name, airport_name
            ),
            format!(
                "💔 DEMAND SHOCK: Economic slowdown kills {} demand at {}",
                cargo_name, airport_name
            ),
            format!(
                "📊 BEARISH: Analysts downgrade {} outlook for {} region",
                cargo_name, airport_name
            ),
        ];
    ...
    fn generate_shortage_description(
        cargo: &str,
        airport: &str,
        airports: &HashMap<String, Airport>,
        cargo_types: &HashMap<String, CargoType>,
        ...
        let scenarios = [
            format!(
                "⚠️ SHORTAGE: Critical {} shortage at {} - limited supply available",
                cargo_name, airport_name
            ),
            format!(
                "🚨 SUPPLY CRISIS: {} production halted - {} faces severe shortage",
                cargo_name, airport_name
            ),
            format!(
                "⛔ RESTRICTED: Export controls limit {} availability at {}",
                cargo_name, airport_name
            ),
            format!(
                "🔒 SCARCITY: Strike action disrupts {} supply chain to {}",
                cargo_name, airport_name
            ),
        ];
    ...
    fn generate_news_description(cargo: &str, is_positive: bool) -> String {
        ...
        if is_positive {
            let scenarios = [
                format!(
                    "📰 BUSINESS NEWS: New technology breakthrough boosts {} industry outlook",
                    cargo_name
                ),
                format!(
                    "🌟 INNOVATION: Revolutionary {} applications drive market optimism",
                    cargo_name
                ),
                format!(
                    "📈 ANALYST UPGRADE: {} sector receives strong buy rating from Wall Street",
                    cargo_name
                ),
                format!(
                    "🚀 GROWTH STORY: {} demand expected to surge on global expansion",
                    cargo_name
                ),
            ];
            scenarios[rand::thread_rng().gen_range(0..scenarios.len())].clone()
            ...
            let scenarios = [
                format!(
                    "📰 MARKET NEWS: Regulatory concerns weigh on {} sector performance",
                    cargo_name
                ),
                format!(
                    "⚠️ INDUSTRY ALERT: Environmental issues impact {} market sentiment",
                    cargo_name
                ),
                format!(
                    "📉 ANALYST DOWNGRADE: {} sector faces headwinds from economic uncertainty",
                    cargo_name
                ),
                format!(
                    "🌧️ CLOUDY OUTLOOK: Global tensions affect {} international trade",
                    cargo_name
                ),
            ];
            scenarios[rand::thread_rng().gen_range(0..scenarios.len())].clone()
    ...
    pub fn update_events(events: &mut Vec<MarketEvent>) -> Vec<String> {
        ...
        events.retain_mut(|event| {
            ...
            if event.turns_remaining == 0 {
                expired_events.push(format!(
                    "Market conditions normalize: {} effects have ended",
