...
async fn test_invalid_session_id_errors() {
    ...
    let response = server
        .post(&format!("/game/{}/travel", fake_session_id), travel_body)
        .await
        .unwrap();
    ...
    let response = server
        .post(&format!("/game/{}/trade", fake_session_id), trade_body)
        .await
        .unwrap();
    assert_eq!(response.status(), 400);
    ...
    let response = server
        .post(&format!("/game/{}/fuel", fake_session_id), fuel_body)
        .await
        .unwrap();
    assert_eq!(response.status(), 400);
}
...
async fn test_insufficient_funds_scenarios() {
    ...
    let response = server
        .post(&format!("/game/{}/fuel", session_id), expensive_fuel)
        .await
        .unwrap();
    assert_eq!(response.status(), 200);
...
async fn test_cargo_capacity_errors() {
    ...
    let response = server
        .post(&format!("/game/{}/trade", session_id), heavy_cargo_trade)
        .await
        .unwrap();
    assert_eq!(response.status(), 200);
...
async fn test_invalid_cargo_operations() {
    ...
    let response = server
        .post(&format!("/game/{}/trade", session_id), invalid_cargo)
        .await
        .unwrap();
    assert_eq!(response.status(), 400);
    ...
    let response = server
        .post(&format!("/game/{}/trade", session_id), sell_nonexistent)
        .await
        .unwrap();
    assert_eq!(response.status(), 200);
...
async fn test_invalid_destination_errors() {
    ...
    let response = server
        .post(&format!("/game/{}/travel", session_id), invalid_destination)
        .await
        .unwrap();
    assert_eq!(response.status(), 400);
    ...
    let response = server
        .post(&format!("/game/{}/travel", session_id), same_destination)
        .await
        .unwrap();
...
async fn test_fuel_capacity_limits() {
    ...
    let fill_response = server
        .post(&format!("/game/{}/fuel", session_id), max_fuel_purchase)
        .await
        .unwrap();
    ...
    let response = server
        .post(&format!("/game/{}/fuel", session_id), excess_fuel)
        .await
        .unwrap();
    assert_eq!(response.status(), 200);
...
async fn test_malformed_request_bodies() {
    ...
    let response = server
        .post(&format!("/game/{}/travel", session_id), malformed_travel)
        .await
        .unwrap();
    ...
    let response = server
        .post(&format!("/game/{}/trade", session_id), malformed_trade)
        .await
        .unwrap();
    ...
    let response = server
        .post(&format!("/game/{}/fuel", session_id), malformed_fuel)
        .await
        .unwrap();
    assert_eq!(response.status(), 422);
}
...
async fn test_fuel_consumption_edge_cases() {
    ...
    let _ = server
        .post(&format!("/game/{}/travel", session_id), consume_fuel)
        .await
        .unwrap();
    ...
    let response = server
        .post(&format!("/game/{}/travel", session_id), long_trip)
        .await
        .unwrap();
...
mod model_error_tests {
    ...
    fn test_distance_calculation_edge_cases() {
        let airport1 =
            kzrk::models::Airport::new("A1", "Airport 1", (0.0, 0.0), 50, vec![], vec![], 1.0);
        let airport2 =
            kzrk::models::Airport::new("A2", "Airport 2", (0.0, 0.0), 50, vec![], vec![], 1.0);
        ...
        let north_pole =
            kzrk::models::Airport::new("NP", "North Pole", (90.0, 0.0), 50, vec![], vec![], 1.0);
        let south_pole =
            kzrk::models::Airport::new("SP", "South Pole", (-90.0, 0.0), 50, vec![], vec![], 1.0);
