...
async fn test_complete_gameplay_flow() {
    ...
    let response = server.post(&format!("/game/{}/fuel", session_id), fuel_body).await.unwrap();
    assert_eq!(response.status(), 200);
    ...
    let response = server.post(&format!("/game/{}/travel", session_id), travel_body).await.unwrap();
    assert_eq!(response.status(), 200);
    ...
    let response = server.post(&format!("/game/{}/trade", session_id), sell_body).await.unwrap();
    assert_eq!(response.status(), 200);
    ...
    assert!(final_state["statistics"]["distances_traveled"].as_f64().unwrap() > 0.0);
}
...
async fn test_error_scenarios() {
    ...
    let response = server.post(&format!("/game/{}/travel", session_id), invalid_travel).await.unwrap();
    ...
    assert!(travel_response["message"].as_str().unwrap().contains("fuel"));
}
...
async fn test_concurrent_sessions() {
    ...
    let (state_response1, state_response2) = tokio::join!(
        server.get(&state_url1),
        server.get(&state_url2)
    );
...
async fn test_game_statistics_tracking() {
    ...
    // Execute actions
    server.post(&format!("/game/{}/fuel", session_id), fuel_purchase).await.unwrap();
    server.post(&format!("/game/{}/trade", session_id), trade_purchase).await.unwrap();
    server.post(&format!("/game/{}/travel", session_id), travel).await.unwrap();
