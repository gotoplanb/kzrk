use kzrk::models::{Player, Airport};
...
mod property_tests {
    ...
    fn test_market_price_boundaries() {
        ...
        for (id, cargo_type) in cargo_types {
            assert!(cargo_type.base_price > 0, "Cargo {} has zero base price", id);
            assert!(cargo_type.base_price < 10000, "Cargo {} has unreasonably high base price: {}", id, cargo_type.base_price);
            
            assert!(cargo_type.weight_per_unit > 0, "Cargo {} has zero weight", id);
            assert!(cargo_type.weight_per_unit < 100, "Cargo {} has unreasonably high weight: {}", id, cargo_type.weight_per_unit);
            
            assert!(cargo_type.volatility >= 0.0, "Cargo {} has negative volatility", id);
            assert!(cargo_type.volatility <= 1.0, "Cargo {} has volatility > 1.0: {}", id, cargo_type.volatility);
        }
    }
    ...
    fn test_airport_data_consistency() {
        ...
        for (id, airport) in airports {
            ...
            assert!(airport.coordinates.0 >= -90.0 && airport.coordinates.0 <= 90.0, 
                "Airport {} has invalid latitude: {}", id, airport.coordinates.0);
            assert!(airport.coordinates.1 >= -180.0 && airport.coordinates.1 <= 180.0,
                "Airport {} has invalid longitude: {}", id, airport.coordinates.1);
            
            // Fuel prices should be reasonable
            assert!(airport.base_fuel_price > 0, "Airport {} has zero fuel price", id);
            assert!(airport.base_fuel_price < 500, "Airport {} has unreasonably high fuel price: {}", id, airport.base_fuel_price);
            
            // Fuel modifier should be positive
            assert!(airport.market_profile.fuel_modifier > 0.0, "Airport {} has non-positive fuel modifier", id);
            assert!(airport.market_profile.fuel_modifier < 5.0, "Airport {} has extremely high fuel modifier: {}", id, airport.market_profile.fuel_modifier);
        }
    }
}